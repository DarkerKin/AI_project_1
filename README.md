...
AI_project_1/
  ...
  train_and_save.py       # Trains the model and saves artifacts to disk
  app.py                 # Flask web app — live predictions for all stocks
  templates/
    index.html           # Web UI: two-column UP/DOWN stock layout
  saved_model/           # Trained model + scaler (generated by train_and_save.py)
...
Each training run fetches data for ~100 tickers, engineers features, builds a leakage-free chronological train/validation split, trains with early stopping, then prints a full evaluation against baseline models plus sample next-day predictions. `main.py` also writes `model_architecture.png`.
