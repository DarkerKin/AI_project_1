...
**Text:**
- Ran the same backtest methodology Fischer & Krauss (2018, *European Journal of Operational Research*) used: rank stocks daily by predicted return, long the top 10, short the bottom 10, measure the realized portfolio return
- Our result: **+0.03% avg. daily return, Sharpe 0.31** (364-day window, 2025–26) vs. their published **+0.46%, Sharpe 5.8** (23-year window, 1992–2015, before transaction costs)
- The gap reflects three real factors, not a methodology failure: (1) a much shorter, statistically noisier test window, (2) their period spans the dot-com crash and 2008 crisis — high-volatility regimes with more short-term price dislocation to exploit, and (3) "alpha decay" — markets tend to arbitrage away a pattern once it's published and adopted
- A Sharpe ratio of 5.8 is itself far above what most professional quant funds sustain long-term (typically 1–3) — an outlier benchmark, not a standard bar
- Separately, a systematic survey of this research area (*Artificial Intelligence Review*) found that of 10,000+ papers reviewed, **only 35 performed proper backtesting** — its conclusion: published accuracy claims in this field **"often overstate practical utility"**
...
**Speaker (2) — this directly answers "how does this compare to real-world models," addresses it head-on. Don't rush past the gap — explain it:**
*"We wanted a real comparison, not just a citation, so we ran the exact backtest methodology this well-known 2018 study used on our own model: each day, rank stocks by predicted return, go long the top 10, short the bottom 10, and measure what actually happened. Our result was +0.03% average daily return with a Sharpe of 0.31; their published result was +0.46% and a Sharpe of 5.8. That's a real gap, and there are three concrete reasons for it, not a flaw in our approach. First, our test window is only 364 trading days, while theirs spans 23 years — Sharpe ratios estimated over short windows are inherently noisy, so this isn't a stable apples-to-apples comparison. Second, their period covered the dot-com crash and the 2008 financial crisis — high-volatility regimes that create far more short-term price dislocation for a model to exploit than the calmer, more recent window we tested on. And third, there's a well-documented effect in quantitative finance called alpha decay: once a pattern gets published and adopted, the market tends to arbitrage it away over time, so a strategy built on data through 2015 had more room to find exploitable structure than the same idea does a decade later. It's also worth noting a Sharpe of 5.8 is far above what most professional quant funds sustain long-term — typically 1 to 3 — so that benchmark itself sits at the extreme high end of what's normally achievable, not a bar every model should be expected to clear. And separately, a systematic survey of this whole research area found that of over 10,000 papers, only 35 actually did proper backtesting, concluding that published accuracy claims in this field often overstate practical utility — our methodology, with its chronological splitting and honest baseline comparisons, is exactly the rigor that survey identifies as rare."*
